...
inline fun <reified T : Any> Context.res(@AnyRes id: Int): ReadOnlyProperty<Activity, T> = ResourcesDelegate(
    this, id, T::class)

inline fun <reified T : Any> Fragment.res(@AnyRes id: Int): ReadOnlyProperty<Activity, T> = ResourcesDelegate(
    this.context, id, T::class)

class ResourcesDelegate<T : Any>(val context: Context, val id: Int,
    val cls: KClass<T>) : ReadOnlyProperty<Any, T> {
  ...
  private fun match(desiredType: String,
      desiredClass: KClass<*>) = desiredType == type && desiredClass.java == cls.java
