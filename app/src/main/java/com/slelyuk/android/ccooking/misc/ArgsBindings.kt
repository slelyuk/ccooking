...
inline fun <reified T : Any> Activity.arg(
    default: T? = null): ReadOnlyProperty<Activity, T?> = ActivityArgsDelegate(default, T::class)

inline fun <reified T : Any> Fragment.arg(
    default: T? = null): ReadOnlyProperty<Fragment, T?> = FragmentArgsDelegate(default, T::class)
...
inline fun <reified T : Activity> Activity.runActivityForResult(b: Bundle? = null, requestCode: Int,
    flags: Int = 0) {
  val i = Intent(this, T::class.java).addFlags(flags)
  if (b != null)
    i.putExtras(b)
...
inline fun <reified T : Activity> Context.runActivity(flags: Int = 0,
    noinline f: BundleBuilder.() -> Unit) {
  val b = bundle(f)
  runActivity<T>(b, flags)
}

inline fun <reified T : Activity> Activity.runActivityForResult(requestCode: Int, flags: Int = 0,
    noinline f: BundleBuilder.() -> Unit) {
  val b = bundle(f)
...
inline fun <reified T : Fragment> FragmentActivity.replaceFragment(@IdRes layoutId: Int,
    args: Bundle? = null) {
...
class ActivityArgsDelegate<out T : Any>(val default: T?,
    val cls: KClass<*>) : ReadOnlyProperty<Activity, T?> {
...
class FragmentArgsDelegate<out T : Any>(val default: T?,
    val cls: KClass<*>) : ReadOnlyProperty<Fragment, T?> {
...
private fun <T : Any> parseExtras(key: String, extras: Bundle, cls: KClass<*>, default: T?): T? {
  ...
  with(extras) {
    return when (cls) {
      ...
      java.lang.Boolean::class -> getBoolean(key,
          if (default != null) default as Boolean else false) as T
      ...
      java.lang.Double::class -> getDouble(key,
          if (default != null) default as Double else 0.0) as T
